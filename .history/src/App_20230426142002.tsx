...
const App = () => {
  ...
  const characters = allCharacters.filter(character => {
    ...
    if (nameFilter !== '' && !character.name.toLowerCase().includes(nameFilter.toLowerCase())) return false;
    return true;
  });
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {isLoading ? (
        <div className='flex justify-center mt-20'><span className="loader"></span></div>
      ) : isError ? (
        <div className='flex justify-center items-center font-bold text-2xl animate-bounce text-white'>404 Error with fetching Data!</div>
      ) : (
        <>
