...
const App = () => {
  ...
  const fetchCharacters = async (_key: string, page: number): Promise<Character[]> => {
    const response = await axios.get<SwapiResponse>(`https://swapi.py4e.com/api/people/?page=${page}`);
  ...
  const { data: characters = [], isLoading, isError } = useQuery<Character[]>(['characters', currentPage], fetchCharacters);
