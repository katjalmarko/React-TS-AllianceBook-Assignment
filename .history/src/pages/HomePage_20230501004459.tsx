...
const HomePage = () => {
  // Declares state variables for pagination, filters, and character data
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [genderFilter, setGenderFilter] = useState<string>('');
  const [nameFilter, setNameFilter] = useState<string>('');
  const [allCharacters, setAllCharacters] = useState<Character[]>([]);

  // Defines a function to fetch character data from the Star Wars API
  const fetchCharacters = async (page: number): Promise<Character[]> => {
    const response = await axios.get(
      `https://swapi.py4e.com/api/people/?page=${page}`
    );

    // Processes the received data and creates a new array with formatted character data
  ...
  const { isLoading, isError } = useQuery<Character[]>(
    ...
    async () => {
      ...
      // Fetches data for each page and accumulate it in the allData array
      for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
        const pageData = await fetchCharacters(pageNum);
        allData.push(...pageData);
      }

      // Updates the allCharacters state with the fetched data
  ...
  // Filters characters based on the applied gender and name filters
  ...
  // Defines a function to change the current page
  ...
  // Resets the current page when the gender filter is changed
  useEffect(() => {
    setCurrentPage(1);
  }, [genderFilter]);
  ...
  return (
    <div className="container mx-auto px-4 py-">
      ...
      <div className="mb-4 flex justify-center flex-wrap gap-2">
        {Array.from({ length: totalPages }, (_, index) => index + 1).map(
          pageNum => (
            <button
              key={pageNum}
              className={`bg-gray-800 text-white text-xs sm:text-base px-2 sm:px-4 py-1 sm:py-2 rounded mx-1 my-1 ${
      ...
      {/* Displays how many characters were found in the database and current page */}
      <div className="flex flex-col sm:flex-row items-center justify-between mx-16 text-center space-y-2 sm:space-y-0 sm:space-x-4 mb-4">
